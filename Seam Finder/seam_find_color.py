...
def union(im1,off1,im2,off2):
    shape = np.abs(np.subtract(np.maximum(np.add(off1, im1), np.add(off2, im2)), np.minimum(off1, off2)))
    shape = tuple(np.append(shape, 3))
    return shape
...
def stitch_image(image, patch, offset, show_seam):
    ...
    image1_overlap_row = image1_row[offset[0]:offset[0] + row_overlap, offset[1]:offset[1] + col_overlap]
    image2_overlap_row = image2_row[:row_overlap, :col_overlap]  # retrieving the overlapped regions in the two images
    ...
    segments = np.dstack([segments] * 3)
    # overlap = np.where(segments, image2_overlap, image1_overlap)
    overlap = np.where(segments, image2_overlap_row, image1_overlap_row)
    #print(segments)
    ...
    res_img[:              image1.shape[0], :              image1.shape[1], :] = image1_row
    res_img[offset[0]:offset[0] + image2.shape[0], offset[1]:offset[1] + image2.shape[1], :] = image2_row
    res_img[offset[0]:offset[0] + overlap.shape[0], offset[1]:offset[1] + overlap.shape[1], :] = overlap
    # res_img[offset[0]:offset[0] + overlap.shape[0], offset[1]:offset[1] + overlap.shape[1], :] = overlap
...
image1_row = cv.imread('night1.png')
image2_row = cv.imread('night2.png')

b,g,r = cv.split(image1_row)
image1_row = cv.merge([r,g,b])
b,g,r = cv.split(image2_row)
image2_row = cv.merge([r,g,b])

# plt.imshow(image1_row)
# plt.show()

#image = stitch_patch(image, patch, (400, 100), 1)
res_image = stitch_image(image1, image2, (50, 700),0) # the offset value has been given manually for now.
...
# plt.savefig('Stiched_building.jpg')
